...
use twilight_gateway::{Config, EventTypeFlags, Intents, MessageSender, Shard, ShardId, StreamExt};
...
pub struct DiscordBotClient {
    shard_message_senders: HashMap<ShardId, Arc<MessageSender>>,
...
unsafe impl Send for DiscordBotClient {}
unsafe impl Sync for DiscordBotClient {}

impl DiscordBotClient {
    pub async fn new(
        runtime: Arc<Mutex<Runtime>>,
        initialized_plugins: InitializedPlugins,
    ) -> Result<(Self, impl ExactSizeIterator<Item = Shard>, Arc<Data>), ()> {
