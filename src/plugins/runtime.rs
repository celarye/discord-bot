...
use crate::{
    ...
    plugins::{
        ...
        InitializedPluginsEvents,
        runtime::{
            discord_bot::plugin::plugin_types::{EventResponse, ScheduledTaskResponse},
...
impl Runtime {
    ...
    pub async fn initialize_plugins(
        ...
        (
            Arc<RwLock<InitializedPlugins>>,
            InitializedPluginRegistrations,
        ),
        (),
    > {
        let mut initialized_plugins = InitializedPlugins {
            events: InitializedPluginsEvents {
        ...
        for plugin in plugins {
                ...
                .call_init(
                    ...
                    &simd_json::to_vec(
                        &plugin
                            .settings
                            .clone()
                            .unwrap_or(simd_json::OwnedValue::default()),
                    )
                    .unwrap(),
                )
                .await
            ...
            if init_result.registered_events.message_create {
                initialized_plugins
                    .events
                    .message_create
            ...
            for interaction_create_command in
                init_result.registered_events.interaction_create_commands
            {
            ...
            for dependency_function in init_result.dependency_functions {
        ...
        Ok((
            Arc::new(RwLock::new(initialized_plugins)),
    ...
    pub async fn call_event(
        ...
        plugin_context
            ...
            .call_event(
                ...
                event,
                &discord_bot::plugin::shared_types::Contexts {
                    host: simd_json::to_vec(&*data.blocking_read()).unwrap(),
                    plugin: plugin_context.internal_context.clone(),
                },
            )
            .await
            .unwrap()
    ...
    pub async fn call_scheduled_task(
        &mut self,
        plugin_name: &str,
        scheduled_task: &str,
        data: Arc<RwLock<Box<Data>>>,
