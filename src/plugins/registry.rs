...
use crate::{config::ConfigPluginValues, http::HttpClient};

use super::AvailablePlugin;
...
pub async fn get_plugins(
    ...
    plugins: &IndexMap<String, ConfigPluginValues>,
    directory: &Path,
    cache: bool,
    ...
    for (plugin_id, plugin_options) in plugins {
        ...
        let plugin_dir = directory.join(name).join(&version);

        if cache && fs::exists(plugin_dir.join("plugin.wasm")).unwrap_or(false) {
        ...
        if let Err(err) = fs::create_dir_all(directory.join(name).join(&version)) {
            error!(
        ...
        if let Err(err) = fs::write(
            directory.join(name).join(&version).join("metadata.json"),
            plugin_metadata,
        ) {
            error!(
        ...
        if let Err(err) = fs::write(
            directory.join(name).join(&version).join("plugin.wasm"),
            plugin_wasm,
        ) {
            error!(
